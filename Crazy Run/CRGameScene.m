...
//         _bground.centerRect = CGRectMake(1,1,1,1);
...
- (void)setPlayer {
    ...
    _player = [SKSpriteNode spriteNodeWithImageNamed:@"ball.png" ];
...
- (void)setBlockToPosition:(CGPoint)position {
    //If your need change color to image
    _block = [SKSpriteNode spriteNodeWithImageNamed:@"star.png"];
...
- (void)setTargetToPosition:(CGPoint)position {
    //If your need change color to image
    _target = [SKSpriteNode spriteNodeWithImageNamed:@"ball.png"];
...
-(void)updateWithTimeSinceLastUpdate:(CFTimeInterval) timeSinceLast{
    if (nowPlay) {
        ...
        if (_lastSpawnTimeInterval > 1) {
            ...
            if ((arc4random() % 2) == 1) {
                [self setTargetToPosition:CGPointMake(arc4random() % (int)self.frame.size.width/1.5, 0)];
            }
            else {
                [self setBlockToPosition:CGPointMake(arc4random() % (int)self.frame.size.width/1.5, 0)];
            }
